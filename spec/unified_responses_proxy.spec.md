...
An upstream Responses `response.output_item.added.item.encrypted_content` value does not satisfy the meaningful-payload predicate because SACC-5a classifies it as provisional. Monoize MUST omit that value from the downstream `response.output_item.added` event. A later completed encrypted snapshot MAY make the same reasoning node meaningful and start its downstream lifecycle with the same item id.
...
- An `encrypted_content` value in `response.output_item.added.item` is provisional. Monoize MUST NOT place it in the reasoning accumulator, compare it with completed encrypted content, or retain it as a fallback when completed encrypted content is absent.
